...
const WEBHOOK_URLS: Array<string> = JSON.parse(process.env.WEBHOOK_URLS as string);
...
function formatComment(comment: string): [string, Array<string>] {
    ...
    urls ? mediaLinks.push(...urls?.map(url => url.endsWith('svg') ? `https://util.bruhmomentlol.repl.co/svg?q=${url}` : url)) : null
